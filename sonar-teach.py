# Sonar Treasure Hunt
# A gane by Late'jah & Miguel & Amanda & Jessica
...
def getNewBoard():
    ...
    for x in range(60): #The main list is a list of 60 lists
        ...
        for y in range(15): #Each list in the main list has 15 single character strings
            #Use different chars for for the ocean to make it more readable
...
def drawBoard(board):
    ...
    tenDigitsLine = '    ' #Initial space for the numbers down the left side of the board
    for i in range(1, 6):
    ...
    for row in range(15):
        ...
        #Create the strring for this row on the board
        boardRow = ''
        for column in range(60):
            boardRow  += board[column][row]

        #print(extraSpace, type(extraSpace))
        #print(row, type(row))
        #print(boardRow, type(boardRow))
...
def getRandomChests(numChests):
    #Create a list of chest data stuctures (two-item lists of x, y int coordinates).
    chests = []
...
def makeMove(board, chests, x, y):
    #Changes the board data structure with a sonar device character. Remove treasure chests from the chests list as they are found,
    #Return False if this is an invalid move
    #Otherwise, return the string of the reuslt of this move.
    smallestDistance = 100 #Any chest will be closer han 100
    for cx, cy in chests:
...
def enterPlayerMove(previousMoves):
    ...
    while True:
        move = input()
        if move.lower().strip() == 'quit': #QUIT or Quit or quit or qUit or ' quit   '
...
while True:
    ...
    while sonarDevices > 0:
        #Show sonar devices and chests statuses.
